...
if __name__ == '__main__':
    ...
    models = ['cot', 'cot_noiseFalse', 'cot_noiseTrue', 'pot', 'pot_noiseFalse', 'pot_noiseTrue']
    ...
    for model in models:
        ...
        try:
            ...
            print(f'No results for {model} found, skipping...')
            continue
        ...
        for i, row in results.iterrows():
            try:
                r_obj = eval(row["response"])
                if isinstance(r_obj, list):
                    row['response'] = ', '.join(map(str, r_obj))
            except:
                pass
            ...
            if range:
                ...
                if 'cot' in model or 'pot' in model:
                    row = preprocessing_range_openai(row, args.dataset)
                else:
                    raise NotImplementedError(f'Preprocessing for range not implemented for model {model}')
        ...
        if args.dataset == 'extra':
            ...
            calculate_and_save_metrics(hierarchical_res, metrics_hier, model, f'./results/{args.type}/{args.dataset}/{model}_metrics_hier.csv')

            print(f'Extra - Not Hierarchical')
            calculate_and_save_metrics(not_hiercahical_res, metrics_not_hier, model,f'./results/{args.type}/{args.dataset}/{model}_metrics_not_hier.csv')

        else:
            calculate_and_save_metrics(results, metrics, model, f'./results/{args.type}/{args.dataset}/{model}_metrics.csv')
